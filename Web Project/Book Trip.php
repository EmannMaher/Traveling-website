<?php
  ...
  $_SESSION['image']=$_COOKIE['img'];
  if (isset($_SESSION['Price']) && isset($_SESSION['date']) &&isset($_SESSION['first'])){
  ...
  <div class="col-75">
    <div class="container">
      <form action="getPrice.php" method="post">
        <div class="row">
          <div class="col-50">
            <h3 style="margin-bottom: 50px; margin-top: 10px;"><i class="fa fa-address-card-o"></i> Booking Trip Info</h3>
            <label for="fdate">Check-in Date</label>
            <input type="date" id="fdate" name="firstdate" value="<?php echo (isset($date))?$date:'';
            unset($date); unset($_SESSION['first']);?>">
            <div class="row">
              <div class="col-50">
                <label for="adults">Adults</label>
                <input type="number" id="adults" name="Adults" style="width: 100%; margin-bottom: 20px; padding: 12px; border: 1px solid bold #000; border-radius: 3px;"value="<?php echo (isset($_SESSION['Adults']))?$_SESSION['Adults']:'';
                unset($_SESSION['Adults']);?>">
              </div>
              <div class="col-50">
                <label for="Children">Children</label>
                <input type="number" id="Children" style="width: 100%; margin-bottom: 20px; padding: 12px; border: 1px solid bold #000; border-radius: 3px;"
                name="Children" value="<?php echo (isset($_SESSION['Children']))?$_SESSION['Children']:'';
                unset($_SESSION['Children']);?>">
              </div>
            </div>
            <div class="row">
              <div class="col-50">
            <button type ="submit" class="totprice" >Get Total Price</button>
              </div>
              <div class="col-50">
              <?php if (isset($price)) { ?>
     		<p class="error" style="background: white; color: black; border: 2px dashed black;"><?php echo $price.' $'; ?></p>
     	<?php  $conf=1; unset($price); } 
      else { ?>
        <p class="error" style="background: white; color: black; border: 2px dashed black;"><?php echo "Total Price"; ?></p>
        <?php  }  ?>
              </div>
            </div>
          ...
          <form action="Booking.php" method="post">
            ...
            <div class="row">
              ...
              <label class="error" id="err" style="background-color:white;"></label>
            </div>
          </div>
        ...
        <?php if($conf==0)
     { ?>  
     <button type="button" onclick =" validate()"name="confirm" class="button" style="background-color: #45f3ff;" disabled>Confirm</button>
     <?php
     }
    else{?>
    <button type="submit" onclick =" validate()" name="confirm"  class="button" style="background-color: #45f3ff;">Confirm</button>
     <?php
    }?>
      </form>
    </div>
